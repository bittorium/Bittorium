...
const command_line::arg_descriptor<uint16_t> arg_daemon_port = { "daemon-port", "Use daemon instance at port <arg> instead of 39984", 0 };
...
bool pool_wallet::confirmTransaction(TransferCommand cmd, bool multiAddress) {
  ...
  /* 10 shells = 0.01 PSTAR */
  if (cmd.fee == 1) {
    feeString = "0.01 PSTAR (minimum)";
  } else {
    feeString = m_currency.formatAmount(cmd.fee) + " PSTAR";
  }
  ...
  if (!multiAddress) {
    std::cout << "You are sending " << m_currency.formatAmount(cmd.dsts[0].amount)
              << " PSTAR, with a fee of " << feeString << std::endl
    ...
    for (auto destination : cmd.dsts) {
      std::cout << "You are sending " << m_currency.formatAmount(destination.amount)
                << " PSTAR" << std::endl << "FROM: " << walletName << std::endl
...
bool pool_wallet::print_outputs_from_transaction(const std::vector<std::string>& args)
{
  ...
  for (size_t i = 0; i < ourTransaction.outputs.size(); ++i)
  {
    ...
    if (targetPubKey.key == outputPublicKey)
    {
      ...
      std::string pstar = m_currency.formatAmount(amount);

      sum += amount;

      found = true;

      logger(INFO, GREEN) << "The transaction output of " << pstar << " PSTAR belongs to you!";
    }
  }
  ...
  {
    std::string pstar = m_currency.formatAmount(sum);

    logger(INFO, GREEN) << "Outputs totalling " << pstar << " PSTAR were sent to your wallet!";
  }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "PinkstarcoinV2 v" << PROJECT_VERSION << " Poolwallet" << std::endl;

  std::cout << "Please note that usage of simplewallet/poolwallet has been "
            << "deprecated for pool usage." << std::endl
            << "If you are using pinkstar-pool, you can trivially transfer to "
            << "walletd by following these instructions:" << std::endl
            << "https://github.com/Jachi84/PinkstarcoinV2.git" << std::endl;
